(async function () {
  ...
  function requestedEvents(date) { return read('sg-requests', []).filter((item) => item.date === iso(date)).map((item, index) => ({ id: `request-${item.id || index}`, team: item.team, from: item.from, to: item.to, place: item.place, originalPlace: item.place, type: typeForCapacity(item.capacity), category: item.status === 'freigegeben' ? 'Freigegebener Termin' : 'Beantragter Termin', note: item.note || (item.status === 'freigegeben' ? 'Freigegeben' : 'Beantragt'), source: 'Abteilungsleitung', pending: item.status !== 'freigegeben', requestStatus: item.status })); }
  ...
  function rawEvents(date) {
    ...
    const current = base.filter((event) => !overrides[event.id] || !overrides[event.id].date || overrides[event.id].date === iso(date)).map((event) => overrides[event.id] ? { ...event, ...overrides[event.id], originalPlace: event.originalPlace, source: `${event.source} · administrativ geändert` } : event);
    const movedHere = Object.entries(overrides).filter(([id, override]) => override.date === iso(date) && !base.some((event) => event.id === id)).map(([id, override]) => ({ id, ...override, originalPlace: override.originalPlace || 'nicht festgelegt', category: 'Administrativ geänderter Termin', source: 'Administrator' }));
  ...
  function dayDetail(date) { const mow = mowingDates.has(iso(date)); const note = (window.Cloud.mowing || []).find((item) => item.date === iso(date))?.note; return `<article class="day-row"><div class="day-label"><b>${dayName(date)}</b><span>${fmtDate(date)}</span></div><div class="mow ${mow ? '' : 'empty'}">${mow ? `Mähen${note ? ` · ${esc(note)}` : ' · Hinweis'}` : ''}</div><div class="timeline"><div class="time-scale">${hourMarks.map((mark, i) => `<span style="top:${i * 100 / 13}%">${mark}</span>`).join('')}</div><div class="lanes">${places.map((place) => lane(date, place)).join('')}</div></div></article>`; }
  function mobileDay(date) {
    ...
    return `<article class="mobile-day"><header><div><strong>${dayName(date)} · ${fmtDate(date)}</strong><span>Sonnenuntergang ${sun.label} · A-Platz bis ${latestLabel}</span></div>${mow ? '<b class="mobile-mow">Mähen</b>' : ''}</header>${places.map((place) => {
