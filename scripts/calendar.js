(async function () {
  ...
  function eventMarkup(event) {
    ...
    const bfvCheck = event.idConflict
      ? `<p class="bfv-conflict">BFV-Abgleich nötig: Datum oder Uhrzeit der manuellen Fassung weichen bei gleicher BFV-ID vom aktuellen BFV-Spiel ab. Der manuelle Termin bleibt erhalten. BFV meldet: ${esc(event.bfvOfficial.date)} · ${esc(event.bfvOfficial.from)}–${esc(event.bfvOfficial.to)}. Eine BFV-Platznummer wird nicht übernommen.</p>`
      : event.bfvMissing
