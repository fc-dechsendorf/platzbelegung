(async function () {
  ...
  function eventMarkup(event) {
    const editable = role === 'admin', requestable = role === 'manager';
    const sun = sunsetFor(new Date(`${event.date}T12:00:00`));
    const bfvCheck = event.idConflict
      ? `<p class="bfv-conflict">BFV-Abgleich nötig: Diese manuelle Fassung und ein BFV-Spiel haben dieselbe ID. Der manuelle Termin bleibt erhalten. BFV meldet: ${esc(event.bfvOfficial.date)} · ${esc(event.bfvOfficial.from)}–${esc(event.bfvOfficial.to)}. Eine BFV-Platznummer wird nicht übernommen.</p>`
      : event.bfvMissing
        ? '<p class="bfv-conflict">BFV-Abgleich nötig: Die ID dieses manuell geänderten Spiels fehlt im aktuellen BFV-Abruf. Der manuelle Termin bleibt erhalten.</p>'
        : '';
    const moveLabel = event.manual ? 'manuell' : 'automatisch';
    const bfvGame = event.id.startsWith('bfv-');
    const idInfo = bfvGame ? `<dt>BFV-ID</dt><dd>${esc(event.uid || event.bfvOfficial?.uid || event.id.slice(4))}</dd><dt>BFV-Platzangabe</dt><dd>Für unsere Platzverteilung nicht maßgeblich; der Platz wird nach Vereinsregeln festgelegt.</dd>` : '';
    const placeOrigin = bfvGame ? 'Planungsstart' : 'Ursprünglicher Platz';
    return `<section class="modal-card"><button class="close" data-close>×</button><span class="eyebrow">${esc(event.category)}</span><h2>${esc(event.team)}</h2>${bfvCheck}<dl><dt>Datum</dt><dd>${esc(event.date)}</dd><dt>Zeit</dt><dd>${esc(event.from)}–${esc(event.to)}</dd><dt>Platz</dt><dd>${esc(event.place)}${event.place !== event.originalPlace ? ` · ${moveLabel} von ${esc(event.originalPlace)} verlegt` : ''}</dd><dt>${placeOrigin}</dt><dd>${esc(event.originalPlace || 'nicht festgelegt')}</dd><dt>Platzbedarf</dt><dd>${event.type === 'special' ? esc(event.scope || 'Sperre') : `${capacityLabel(event.type)} (${capacity(event.type)} Platzkapazität)`}</dd><dt>Flutlicht</dt><dd>${event.place === 'A' ? `Nein · Sonnenuntergang ${sun.label}` : 'Ja verfügbar'}</dd><dt>Ort</dt><dd>${esc(event.location || 'Sportanlage Dechsendorf')}</dd>${idInfo}<dt>Quelle</dt><dd>${esc(event.source)}</dd><dt>Status</dt><dd>${esc(event.note || 'Standardbelegung')}</dd></dl>${editable ? `<div class="modal-actions">${event.type === 'special' ? '' : '<button class="primary" data-edit-event>Termin bearbeiten</button>'}<button class="secondary" data-delete-event-direct>${event.type === 'special' ? 'Sperre entfernen' : 'Termin löschen'}</button></div>` : requestable && event.type !== 'special' ? '<div class="modal-actions"><button class="primary" data-move-request>Verlegung beantragen</button><button class="secondary" data-delete-request-event>Löschung beantragen</button></div>' : '<p class="access-note">Für Änderungen bitte anmelden.</p>'}</section>`;
  }
